...
class mainMenu {
    ...
    public function write() 
    {
      ...
      if($this->My_Tasks) {
          ...
          $html = $this->writeOne($html, 'My_Tasks', '../trie_table_statut/page_table.php', "?statut=to_treat");
      }
      if($this->Open_Tasks) {
          if ($this->position == "undefined") { $this->position = "Open_Tasks"; }
          $html = $this->writeOne($html, 'Open_Tasks', '../trie_table_statut/page_table.php', "?statut=undefined");
      }
      if($this->Processed_Tasks) {
          if ($this->position == "treat") { $this->position = "Processed_Tasks"; }
          $html = $this->writeOne($html, 'Processed_Tasks', '../trie_table_statut/page_table.php', "?statut=treat");
      }
      if($this->Rejected_Tasks) {
          if ($this->position == "reject") { $this->position = "Rejected_Tasks"; }
          $html = $this->writeOne($html, 'Rejected_Tasks', '../trie_table_statut/page_table.php', "?statut=reject");
      }
      if($this->Insertion) {
